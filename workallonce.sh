...
#   stop_threads = False;   
   ...
   #myid = sys.argv[1]
      myid = str(i)
      cname = cname + myid
      #subprocess.run(['docker-compose', 'run', '-d', cname, '--entrypoint', 'bash bodytrack.sh', 'APP'], stdout=subprocess.PIPE)
      subprocess.run(['docker-compose', 'run', '-d', cname, '--entrypoint', 'bash sleep.sh', 'APP'], stdout=subprocess.PIPE)
